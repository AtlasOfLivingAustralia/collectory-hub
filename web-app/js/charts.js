...
function loadFacetCharts(chartOptions) {
    ...
    $.ajax({
      url: urlConcat(biocacheServicesUrl, "/occurrences/search.json?flimit=1000&pageSize=0&q=") + query,
      dataType: 'jsonp',
      error: function() {
        cleanUp();
      },
...
function drawFacetCharts(data, chartOptions) {
    ...
    var chartsDiv = $('#' + (chartOptions.targetDivId ? chartOptions.targetDivId : 'charts'));
    var query = chartOptions.query ? chartOptions.query : buildQueryString(chartOptions.instanceUid);
    if(chartOptions.isDataHub){
        query += '&' + chartOptions.dataHubFq
    }
...
function initTaxonTree(treeOptions) {
    ...
    if(treeOptions.isDataHub){
        query += '&' + treeOptions.dataHubFq
    }
    biocacheWebappUrl = treeOptions.biocacheWebappUrl || biocacheWebappUrl;
